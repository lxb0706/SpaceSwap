...
> - **本轮目标**: 单视频闭环 + 技术打底；已实现基础全局压缩队列（同会话单视频仅允许压缩一次），复杂批处理能力属于后续版本。
...
- 全局压缩队列（最大并发 10）与队列面板；同一视频在同一 App 生命周期内仅允许压缩一次。
- iCloud 识别与下载（按需下载，支持进度与取消）。
- 基础设置: 扫描阈值、默认压缩预设、压缩后是否自动提示删除原片。

**Out-of-scope（明确不做）**
- 批量多选编排（批量勾选、批量下发、批量策略模板）与高级重试调度。
...
| 压缩队列 | 全局队列与状态回跳 | 点击压缩后进入全局队列；首页再次点击同视频时若处于 `queued/running` 直接进入队列面板 | 最大并发 10；同一 `assetID` 同会话不可重复入队；状态与进度可见 | 支持队列中取消；失败/取消保持会话锁定，避免重复压缩导致状态混乱 | 成功后落库 `CompressionRecord`；队列状态保存在会话内存 |
...
*   **视频扫描 (Scanner)**:
    ...
    *   **性能策略**:
        *   扫描在后台队列执行，主线程仅接收最终结果（当前实现为一次性返回，不做增量流式渲染）。
        *   进度条采用过程态反馈，结果落地时一次性刷新列表。
    *   **取消策略（收敛）**:
        *   点击取消后立即停止扫描，并复位扫描状态与结果列表（当前实现口径）。
*   **视频压缩 (Compressor)**:
...
*   **视频预览（当前实现口径）**:
    *   详情页提供可播放预览，容器最长边 = 屏幕宽度 - 左右间距，横竖屏按视频原始宽高比自适应。
    *   默认自动播放 + 循环播放；点击预览区域可播放/暂停切换。
    *   采用 `VideoPlayer` 系统控件（包含系统进度条与拖动能力），不额外叠加自定义进度条。
    *   右上角提供“放大/缩小”按钮（`viewfinder.circle.fill` / `xmark.circle.fill`）。
    *   全屏不是底部弹出页，而是**同页面过渡动画**（预览位置 <-> 全屏位置）；全屏态隐藏导航栏返回按钮，避免交互干扰。
    *   `Video Details` 展示用户可读信息：文件名、原始大小、时长、分辨率（含比例）、方向、创建时间、存储位置（本地/iCloud）。
    *   位置信息默认展示为“地名/地址”（反向地理编码），不是经纬度；并在详情中提供地图标注视图。
    *   `More Details` 为完整信息面板（Basic / Location / Technical / Expected To Keep），避免仅显示难理解的原始数值。
*   **布局与滚动（当前实现口径）**:
    *   主页面精简为关键信息卡片（预览 + 基础详情 + 压缩设置 + 状态/动作）。
    *   其余信息放入 `More Details`，避免主视图过长。
    *   页面使用 `ScrollView`，小屏设备超过一屏时可滚动，不截断操作区。
*   **压缩态区分（当前实现口径）**:
    *   详情页明确标识 “Showing Original Video / Showing Compressed Video”。
    *   压缩成功后隐藏“开始压缩”按钮，展示压缩结果信息并优先预览压缩后视频。
...
*   详情页预览: 点击可暂停/恢复；系统进度条可拖动 seek；默认循环播放。
*   全屏交互: 放大后视频铺满全屏区域；缩小时回到原预览位；转场为同页面动画，不出现底部弹窗感。
*   详情信息可读性: 地理信息显示为地名并可查看地图；`More Details` 提供完整 metadata 分组，关键指标以可读口径展示。
*   队列行为: 同一视频若已在 `queued/running`，从首页再次点击会直接进入队列页；不会重复创建任务。
*   已压缩态: 首页列表显示 `Compressed` 标记；再次进入详情页默认展示压缩后预览，且不显示压缩按钮。
